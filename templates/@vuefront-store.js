...
export default (ctx, inject) => {
  ...
  modules = _.set(modules, <%= JSON.stringify(options.themeOptions.store[key].path) %>, { namespaced: true, ...<%= key %>})<% } else { %>
    modules = _.set(modules, <%= JSON.stringify(options.themeOptions.store[key].path) %>, { namespaced: true})<% } %><% } %>
